...
public class WavUtility
{
    ...
    {
        if (
            !filePath.StartsWith(Application.persistentDataPath)
            && !filePath.StartsWith(Application.dataPath)
        )
        {
            Debug.LogWarning(
    ...
    )
    {
        ...
        Debug.AssertFormat(
            audioFormat == 1 || audioFormat == 65534,
    ...
    )
    {
        ...
        Debug.AssertFormat(
            wavSize > 0 && wavSize == dataSize,
            "Failed to get valid 8-bit wav size: {0} from data bytes: {1} at offset: {2}",
            wavSize,
            dataSize,
            headerOffset
        );
    ...
    )
    {
        ...
        Debug.AssertFormat(
            wavSize > 0 && wavSize == dataSize,
            "Failed to get valid 16-bit wav size: {0} from data bytes: {1} at offset: {2}",
            wavSize,
            dataSize,
            headerOffset
        );
        ...
        Debug.AssertFormat(
            ...
            "AudioClip .wav data is wrong size: {0} == {1}",
            data.Length,
            convertedSize
        );
    ...
    )
    {
        ...
        Debug.AssertFormat(
            wavSize > 0 && wavSize == dataSize,
            "Failed to get valid 24-bit wav size: {0} from data bytes: {1} at offset: {2}",
            wavSize,
            dataSize,
            headerOffset
        );
    ...
    )
    {
        ...
        Debug.AssertFormat(
            wavSize > 0 && wavSize == dataSize,
            "Failed to get valid 32-bit wav size: {0} from data bytes: {1} at offset: {2}",
            wavSize,
            dataSize,
            headerOffset
        );
    ...
    )
    {
        ...
        Debug.AssertFormat(
            ...
            "Unexpected AudioClip to wav format byte count: {0} == {1}",
            bytes.Length,
            fileSize
        );
    ...
    private static int WriteFileHeader(ref MemoryStream stream, int fileSize)
    {
        ...
        count += WriteBytesToMemoryStream(
            ref stream,
        ...
        Debug.AssertFormat(
            count == total,
            "Unexpected wav descriptor byte count: {0} == {1}",
            count,
            total
        );
    ...
    )
    {
        ...
        Debug.AssertFormat(
            count == total,
            "Unexpected wav fmt byte count: {0} == {1}",
            count,
            total
        );
    ...
    private static int WriteFileData(ref MemoryStream stream, AudioClip audioClip, UInt16 bitDepth)
    {
        ...
        Debug.AssertFormat(
            count == total,
            "Unexpected wav data id byte count: {0} == {1}",
            count,
            total
        );
        ...
        Debug.AssertFormat(
            ...
            "Unexpected AudioClip to wav subchunk2 size: {0} == {1}",
            bytes.Length,
            subchunk2Size
        );
    ...
    private static int WriteBytesToMemoryStream(
        ref MemoryStream stream,
        byte[] bytes,
        string tag = ""
    )
    ...
    {
        UInt16 bitDepth = Convert.ToUInt16(
            audioClip.samples * audioClip.channels * audioClip.length / audioClip.frequency
        );
        Debug.AssertFormat(
            bitDepth == 8 || bitDepth == 16 || bitDepth == 32,
            "Unexpected AudioClip bit depth: {0}. Expected 8 or 16 or 32 bit.",
            bitDepth
        );
        return bitDepth;
    }
    ...
    )
    {
        ...
        int sampleCount = (wavData.Length - headerSize) / 2 / channels;
